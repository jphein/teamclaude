...
| `warmOnExhaustion` | [Keep-warm](quota.md#keep-warm) on exhaustion: when any account uses up its 5h window, warm every cold account right away so the windows line up (`false` = off, the default; CLI `warmup exhaustion on\|off`). **Spends a little quota** per cold account per rotation |
| `warmTransport` | How keep-warm sends its minimal request: `direct` (the proxy POSTs to its own pinned path; the default) or `claude` (spawn a one-shot `claude` CLI pointed at the pinned path) |
| `noProxy` | Extra `NO_PROXY` entries (string or array) that `teamclaude env` always merges into the emitted MITM env — hosts and CIDRs that must never go through the proxy (a homelab's VLANs, internal domains). The shell's own `NO_PROXY` is merged too |
