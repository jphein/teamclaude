...
export class AccountManager {
  ...
  _select(exclude, model, advisorModel, allowProbe) {
    ...
    // Fork diagnostic: say WHY the current account is being left, but only for
    // an outright bar — a model-scoped one is logged by _selectNext as a
    // diversion, and two lines for one decision read as two decisions.
    if (current && !diverted && !this._isModelScopedSkip(current, model, advisorModel)) {
      this._logSkipReason(current, model, advisorModel, exclude);
    }
  ...
  _logSkipReason(account, model, advisorModel, exclude, detour = null) {
    ...
    const forWhat = model ? `model "${safeLine(model, 64)}"` : 'request (model unknown)';
    const adv = advisorModel ? ` advisor "${safeLine(advisorModel, 64)}"` : '';
    const sid = this._skipLogSessionId ? ` [session ${String(this._skipLogSessionId).slice(0, 8)}]` : '';
    const tail = detour
      ? ` — ${why}; Diverting this request to "${detour.name}", "${account.name}" stays current`
      : ` — ${why}`;
  ...
  _selectNext(exclude = null, model = null, advisorModel = null) {
    ...
    if (best) {
      ...
      if (switched) {
        this._beginRamp(best);
        // A diversion is logged as the fork's skip line: it names the account,
        // the model (and advisor), the session and the bucket that barred it,
        // then says the request is diverted and the pointer stays.
        if (scoped) this._logSkipReason(current, model, advisorModel, exclude, best);
        else console.log(`[TeamClaude] Switched to account "${best.name}"`);
      }
      return best;
    }
