...
export function createProxyServer(accountManager, config, hooks = {}, sx = null, clientUsage = null, dimensionUsage = null) {
  ...
  const requestHandler = async (req, res) => {
    try {
      ...
      if (req.method === 'POST' && req.url === '/teamclaude/probe') {
        ...
        try {
          // How many OAuth accounts were eligible, so the dashboard can say
          // "probed N account(s)".
          const probed = accountManager.accounts.filter(a => a.type === 'oauth' && a.credential).length;
          await hooks.probeQuota();
          emitActivity({ type: 'probe', value: probed });
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ ok: true, probed }));
        } catch (err) {
      ...
      if (req.method === 'POST' && req.url === '/teamclaude/switch') {
        ...
        emitActivity({ type: 'switched', account: name });
  ...
  const forward = createProxyRequestListener({ accountManager, upstream, logDir, hooks: channelHooks('h'), sx, holdMs, config, egress, clientUsage, dimensionUsage });
  ...
  server.on('connect', createConnectHandler({ config, accountManager, ensureLeaf, logDir, hooks: channelHooks('m'), log: console.error, sx, egress, clientUsage, dimensionUsage }));
